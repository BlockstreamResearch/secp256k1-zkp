...
void test_ecmult_multi(secp256k1_scratch *scratch, secp256k1_ecmult_multi_func ecmult_multi) {
    ...
    for (ncount = 0; ncount < count; ncount++) {
        ...
        CHECK(!ecmult_multi(&ctx->ecmult_ctx, scratch, &ctx->error_callback, &r, &szero, ecmult_multi_false_callback, &data, 1));
...
void test_ecmult_multi_batching(void) {
    ...
    CHECK(!secp256k1_ecmult_multi_var(&ctx->ecmult_ctx, scratch, &ctx->error_callback, &r, &scG, ecmult_multi_callback, &data, 1));
    ...
    for(i = 1; i <= ECMULT_PIPPENGER_THRESHOLD+2; i++) {
        ...
        CHECK(secp256k1_ecmult_multi_var(&ctx->ecmult_ctx, scratch, &ctx->error_callback, &r, &scG, ecmult_multi_callback, &data, i));
    ...
    for(i = 0; i < (int)(sizeof(test_n_points) / sizeof(test_n_points[0])); i++) {
        secp256k1_gej ptgj;
        CHECK(secp256k1_ecmult_multi_var(&ctx->ecmult_ctx, scratch, &ctx->error_callback, &r, &scG, ecmult_multi_callback, &data, test_n_points[i]-1));
        secp256k1_gej_set_infinity(&r2);
        secp256k1_gej_add_var(&r2, &r2, &r, NULL);
        CHECK(secp256k1_ecmult_multi_var(&ctx->ecmult_ctx, scratch, &ctx->error_callback, &r, &scG, ecmult_multi_callback, &data, test_n_points[i]));
